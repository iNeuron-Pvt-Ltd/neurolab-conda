...
from path_name_provoiders.all_hyperPara import all_hyper_parameter_dic
from path_name_provoiders.all_names import best_parameter as best_parameter
import pandas as pd
import numpy as np
...
class hyper_parameter_classifier:
  ...
  def _return_best_hyper_para(self,model:object,model_name:str,x:pd.DataFrame,y:pd.DataFrame)->dict:
    ...
    try:
        ...
        Random_Search = RandomizedSearchCV( model,para_dic, n_iter=5, cv=5, n_jobs=1, verbose=5)
